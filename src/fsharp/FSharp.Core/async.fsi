...
namespace Microsoft.FSharp.Control
    ...
    type Async =
        ...
        /// <summary>Creates an asynchronous computation that executes <c>computation</c>.
        /// If this computation completes successfully then return <c>Choice1Of3</c> with the returned
        /// value. If this computation is cancelled before it completes then the computation generated by 
        /// running <c>compensation</c> is executed. If the compensation computation completes successfully then return <c>Choice3Of3</c> with the
        /// returned value. If either of these computations raise an exception before they complete then return <c>Choice2Of3</c> with the raised
        /// exception. </summary>
        ///
        /// <param name="computation">The input computation that returns the type T.</param>
        /// <param name="compensation">The function to be run if the computation is cancelled.</param>
        ///
        /// <returns>A computation that returns a choice of computation type T, exception or computation type U.</returns>
        ///
        /// <category index="3">Cancellation and Exceptions</category>
        static member CatchOrCancelled : computation:Async<'T>  * compensation:(OperationCanceledException -> 'U)-> Async<Choice<'T,exn,'U>>
